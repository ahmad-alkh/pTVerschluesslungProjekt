...
public class ECBCode {
    ...
    static char[] blocksToBits(char[][] blocks) {
         ...
         * finaleWert fangt mit an[0]
         */

        int tempArrayLaenge = blocks[0].length * blocks.length;
        char[] finaleWert = new char[tempArrayLaenge];
        int counter = 0;
        for (int i = 0; i < blocks.length; i++) { // erste Array (Bloecke) loopen [0]=110
            for (int j = 0; j < blocks[i].length; j++) { // zweite Array (bits) loopen [0][0]= 1; [0][1]= 1; [0][2]= 0
